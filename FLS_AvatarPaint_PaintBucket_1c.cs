 ...
 *
 * Makes avatar's materials tint to the specified colors when touching the TriggerVolume associated each from the two lists;  The Cleansing Area Trigger will restore avatar materials to their original state from when they joined the scene/instance
 *
...
public class FLS_AvatarPaint_PaintBucket_1c : SceneObjectScript
{
	[DefaultValue(false)]
	...
	public override void Init() {
		...
		CleanserAreaTrigger.Subscribe(CollisionEventType.Trigger, OnTriggerClean);
		RgbTrigger.Subscribe(CollisionEventType.Trigger, OnTriggerRGB);
